...
Source of truth: `iOS/fastlane/screenshots/<locale>/`, generated by
`make appstore-screenshots` from `iOS/App/ScreenshotHarness.swift`. Flat
per-locale layout — no device-size subfolder, because fastlane deliver's
loader ignores subdirs (see QUIRKS.md → "Fastlane deliver ignores
device-size subfolders" and issue #95).
