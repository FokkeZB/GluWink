...
### Cloud widgets fetch from inside the timeline provider
Unlike `ShieldConfig` / `ShieldAction` / `DeviceActivityMonitor`, **WidgetKit extensions DO have network access** — the no-network rule above applies only to Screen Time extensions. We use that to keep widgets fresh when `BGAppRefreshTask` doesn't fire: every `getTimeline` / `snapshot` on the iPhone `StatusWidget` calls `WidgetNightscoutRefresh`, `WidgetEasyViewRefresh`, and `WidgetLibreLinkUpRefresh` in turn, and each writes glucose/carbs back to the App Group with "save if newer" semantics. Guards (per source): skipped when that source is disabled or mock mode is on, throttled to one fetch per `<source>LastFetchedAt + 60s` so snapshot/placeholder/timeline calls coalesce, and bounded by a 5s per-request timeout so a flaky server can't burn the timeline budget.

**Credentials gotcha — the widget cannot re-authenticate.** The widget extension has no Keychain access, so it can only fetch with auth material the main app has copied into the App Group:
- **Nightscout** — URL + token are already App Group settings, so the widget always has what it needs.
- **EasyView** — the main app mirrors the session cookie (`easyViewSession`); when it expires the widget serves stale values and waits for the app to re-login.
- **LibreLinkUp** — email/password stay Keychain-only, but `LibreLinkUpManager.mirrorAuthToAppGroup()` copies the short-lived bearer token + expiry into `librelinkupToken` / `librelinkupTokenExpiry`. `WidgetLibreLinkUpRefresh` fetches `/llu/connections` with that token and **never re-logs in** — at/near expiry (or on a 401) it serves stale values until the app re-authenticates. The shared request/parsing lives in `SharedKit.LibreLinkUpClient` so the app manager and the widget run identical code.

iOS-imposed limits that remain: WidgetKit timeline calls have a budget of ~30s end-to-end, and iOS still decides when to ask us for a fresh timeline (we hint with `.atEnd` plus several entries spaced 1 minute apart, but the system can defer reloads when the device is in low-power mode or our refresh budget is exhausted).
