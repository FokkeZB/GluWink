...
- **Binary / pipe-filter inspection**: `grep`, `strings`, `nm`. `grep` is a pipe filter over arbitrary stdout (the `Grep` tool only works on workspace text files); `strings` and `nm` are for peeking at symbols and embedded strings inside built `.appex` / `.dylib` products — e.g. when verifying SwiftUI watch complication code paths actually got linked. All read-only.
