...
- **Family Controls Distribution** entitlement attached to team `Y39937U7XN` — request at https://developer.apple.com/contact/request/family-controls-distribution/. As of 2026-04 the form is self-serve: attest under the Apple Developer License Agreement that GluWink's primary purpose matches one of the two allowed buckets (parental supervision via Family Sharing, or individual personal device-usage management) and click *Get Entitlement*. The capability is attached to the team within minutes — no human review at this step. Apple still evaluates the shipped app at App Store Review under guideline 5.5, so make sure App Review notes spell out the medical use case (see Production checklist below). Without the entitlement attached, archive succeeds but `xcodebuild -exportArchive` fails because the auto-generated **Store** profile cannot include `com.apple.developer.family-controls`. See `QUIRKS.md` → "Family Controls Distribution entitlement is self-serve, but App Store Review still gates it" for the full story.
